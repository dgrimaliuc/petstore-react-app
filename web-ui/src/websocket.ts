...
export function disconnect() {
  if (websocket) {
    ...
    if (websocket.readyState < CLOSING) websocket.close();
    websocket = null;
  }
}
...
export function onMessage(
  id: string,
  cb: (json: any, websocket: WebSocket) => void
) {
  if (idToListener[id]) {
  ...
  idToListener[id] = function (msg: any) {
    try {
      if (websocket) cb(JSON.parse(msg.data), websocket);
    } catch (e) {
      console.error(e);
    }
  };
