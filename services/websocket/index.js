...
const KAFKA_HOSTS = "https://petstore-kafka.swagger.io/";
  ...
  try {
    ...
    await consumer.run({
      ...
      eachMessage: async ({ topic, partition, message }) => {
        ...
        if (!location) {
          console.error(
            `Log doesnt have location or cached location ${JSON.stringify(
              log
            )} ${topic}`
          );
          return;
    ...
    try {
      ...
      if (!msg.location) {
        socket.send(
          JSON.stringify({
            type: "handshake.ack",
            ok: false,
            reasons: ["Missing .location field in handshake"],
          })
        );
        return;
      }
      ...
      socket.send(
        JSON.stringify({
          type: "handshake.ack",
          ok: true,
          reasons: [`Successfully subscribed to "${location}" changes`],
        })
      );
    } catch (e) {
      console.error(e);
