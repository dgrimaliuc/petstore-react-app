...
module.exports = class KafkaSink {
  constructor({ name, topics, onLog, kafka, makeIdFn, basePath = __dirname }) {
    ...
    this.name = name;
    this.admin = this.kafka ? this.kafka.admin() : undefined;
    this.consumer = this.kafka
      ? this.kafka.consumer({ groupId: this.name })
      : undefined;
  ...
  async seek() {
    return Promise.all(
      this.topics.map(async (topic) => {
        ...
        this.info(
          `Cache does not exist for ${topic}.partition-0. Seeking to beginning...`
        );
        const partitions = await this.admin.fetchTopicOffsets(topic);
        const low = (partitions.find((p) => p.partition === 0) || { low: 0 })
          .low;
        this.info(
          `Low is ${low}. Offsets for ${topic} are ${JSON.stringify(
            partitions
          )}`
        );
  ...
  info(...msgs) {
    console.info(null, { extra: [this.name, ...msgs] });
  }

  error(...msgs) {
    console.error(null, { extra: [this.name, ...msgs] });
  }
