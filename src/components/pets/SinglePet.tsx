...
export default function SinglePet({ pet }: any) {
  ...
  return (
    ...
    >
      ...
      <td className='px-4 py-4 whitespace-nowrap'>
        <span
          className='px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800 uppercase'
          data-t='pet-status'
        >
          {pet.status}
        </span>
      ...
      <td className='px-4 py-4 whitespace-nowrap'>
        <div className='flex items-center justify-end'>
          <div
            className='flex-shrink-0 h-4 w-4 text-gray-600 relative'
            data-t='checkbox'
          >
            {selectedRows.has(pet.id) && (
              <CheckIcon
                className='absolute h-5 text-white left-[-1px] top-[3px]'
                data-t='checked-icon'
              />
            )}
            <input
