...
export function Pill({
  ...
  return (
    <span
      className={`${colorClasses} px-2 uppercase inline-flex text-xs leading-5 rounded-full ${className}`}
      data-t='status-text'
      {...props}
    />
  );
...
export const Button = ({
  ...
  color = 'blue',
  size = 'xs',
  testAttribute = 'button',
  ...props
}: IButton & { testAttribute: string }) => {
  ...
  return (
    <div className='inline-flex space-x-2 justify-center'>
      <button
        data-t={testAttribute}
        className={`${className} ${colorClasses} ${sizeClasses} disabled:bg-gray-300 disabled:text-gray-500 inline-block px-3 py-1.5  text-white font-medium leading-tight uppercase rounded focus:outline-none focus:ring-0  active:shadow-lg transition duration-150 ease-in-out`}
        {...props}
      />
    </div>
  );
